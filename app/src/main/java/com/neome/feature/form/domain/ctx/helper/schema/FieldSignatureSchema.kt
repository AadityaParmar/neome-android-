...
import com.neome.api.meta.base.dto.DefnFieldSignature
...
/**
 * CompSchema implementation for FieldSignature.
 *
 * Uses Konform library for type-safe, declarative validation.
 * Validations:
 * - required: signature must be defined
...
class FieldSignatureSchema(
    ...
    override fun validate(fieldValue: JsonElement?, fieldState: FieldState?): String? {
        ...
        val typedValue = try {
            JsonParser.json.decodeFromJsonElement(
                FieldValueSignatureData.serializer(),
                fieldValue
            )
        } catch (e: Exception) {
            null
    ...
    private fun buildValidation(properties: FieldProperties): Validation<FieldValueSignatureData?> {
        return Validation {
            // Required check - signature must be defined
            if (properties.required) {
                constrain("Required") {
                    it != null && it.signature.isNotBlank()
                }
            }
