...
4. **Condition evaluation** — `ConditionResolver.resolve` evaluates a `DefnEventConditionMap` (AND/OR tree of conditions). Each condition compares a field's value against expected values using operators like `hasValue`, `equalTo`, `greaterThan`, `contains`, `between`, etc. Handles date/time comparisons, set-type fields, and context binder caller resolution.
