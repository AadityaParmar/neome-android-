...
/**
 * CompSchema implementation for FieldDocument.
 *
...
class FieldDocumentSchema(
    ...
    override fun validate(fieldValue: JsonElement?, fieldState: FieldState?): String? {
        ...
        val typedValue = try {
            JsonParser.json.decodeFromJsonElement(
                FieldValueDocumentData.serializer(),
                fieldValue
            )
        } catch (e: Exception) {
            null
    ...
    private fun buildValidation(properties: FieldProperties): Validation<FieldValueDocumentData?> {
        return Validation {
            // Required check
            ...
            properties.maxSize?.let { maxSizeMb ->
                ...
                constrain("File size must be less than $maxSizeMb MB") { value ->
                    ...
                    val fileSize = value.value.fileSize ?: return@constrain true
