...
import com.neome.feature.form.presentation.components.base.TextSizeProvider
...
/**
 * PickTextSize field component for form — single-select dropdown picker.
 *
 * Stateful wrapper that uses rememberFieldController for state management.
 * Delegates rendering to stateless FieldPickTextSizeContent for optimal recomposition.
 ...
 * option's metaId.
 *
 * Uses EnumDefnTextSize for options:
 * - metaId: enum value (e.g., "body1", "h1")
 * - value: capital case (e.g., "BODY1", "H1")
 *
 ...
 */
@Composable
fun FieldPickTextSize(
    defnComp: DefnCompSeal,
    onFieldEvent: (FieldEvent) -> Unit,
    modifier: Modifier = Modifier
) {
    // Cast to specific definition type - support both DefnFieldTextSizeData and DefnFieldPickEnumData
    val defn = defnComp as? DefnFieldTextSizeData
    ...
    // Get filterOptionSet if available (DefnFieldPickEnumData has it, DefnFieldTextSizeData doesn't)
    val filterOptionSet = (defn as? DefnFieldPickEnumData)?.filterOptionSet

    // Resolve option map from EnumDefnTextSize
    val optionMap = TextSizeProvider.getFilteredOptionMap(filterOptionSet)

    // Delegate to stateless content for optimal recomposition
    FieldBase(modifier = modifier, properties = properties) {
        FieldPickTextSizeContent(
            optionMap = optionMap,
...
/**
 * Stateless pickTextSize field content for optimal recomposition control.
 *
 * Wraps [RawPickerSingleSelect] with form-specific error and helper text handling.
 * Only recomposes when its parameters change.
 *
 * @param optionMap Map of available options (from EnumDefnTextSize)
 ...
 */
@Composable
private fun FieldPickTextSizeContent(
