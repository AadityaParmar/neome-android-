...
import com.neome.api.meta.base.dto.DefnFieldLocation
...
/**
 * CompSchema implementation for FieldLocation.
 *
 * Uses Konform library for type-safe, declarative validation.
 * Validations:
 * - required: value must be present with geoPoint
 */
class FieldLocationSchema(
    override val defnForm: DefnFormUi,
    override val defnComp: DefnCompSeal
) : CompSchema(defnForm, defnComp) {

    private val defnField = defnComp as DefnFieldLocation
    ...
    override fun validate(fieldValue: JsonElement?, fieldState: FieldState?): String? {
        ...
        val typedValue = try {
            JsonParser.json.decodeFromJsonElement(
                FieldValueLocationData.serializer(),
                fieldValue
            )
        } catch (e: Exception) {
            null
    ...
    private fun buildValidation(properties: FieldProperties): Validation<FieldValueLocationData?> {
        return Validation {
            // Required check - value must be present (geoPoint is always non-null in parsed value)
