...
/**
 * CompSchema implementation for FieldImage.
 *
...
class FieldImageSchema(
    ...
    override fun validate(fieldValue: JsonElement?, fieldState: FieldState?): String? {
        ...
        val typedValue = try {
            JsonParser.json.decodeFromJsonElement(
                FieldValueImageData.serializer(),
                fieldValue
            )
        } catch (e: Exception) {
            null
    ...
    private fun buildValidation(properties: FieldProperties): Validation<FieldValueImageData?> {
        return Validation {
            // Required check
