...
class FieldColorSchema(
    ...
    override fun validate(fieldValue: JsonElement?, fieldState: FieldState?): String? {
        ...
        val typedValue = try {
            JsonParser.json.decodeFromJsonElement(
                FieldValueColorData.serializer(),
                fieldValue
            )
        } catch (e: Exception) {
            null
    ...
    private fun buildValidation(properties: FieldProperties): Validation<FieldValueColorData?> {
        return Validation {
            // Required check
            if (properties.required) {
                constrain("Required") {
                    it != null && it.value.isNotBlank()
                }
            }
        }
    }
